...
WITH w_n AS (
  ...
  AND de1.drug_exposure_id != de2.drug_exposure_id
  ORDER BY person_id ASC
) SELECT w_n.*, de.drug_exposure_id AS pm_dexp, de.drug_concept_id AS pm_dexp, de.drug_exposure_start_datetime AS pm_start, de.drug_exposure_end_datetime AS pm_end, de.quantity, de.sig, de.route_concept_id, de.route_source_value
FROM drug_exposure de
INNER JOIN w_n ON w_n.person_id = de.person_id
WHERE de.drug_exposure_start_datetime <= w_n.warf_start && de.drug_exposure_end_datetime >= w_n.warf_start
