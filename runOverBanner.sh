...
do
    ...
    echo INFO: Output for this run will be written to epi-beta-blocker-run-$1/output-$i/idia-rules-run-$i.txt;
    mkdir epi-beta-blocker-run-$1/output-$i;
    java -Xmx2048m  -jar target/droolstest-1.0.jar $i > epi-beta-blocker-run-$1/output-$i/idia-rules-run-$i.txt
    sleep 1    
done
