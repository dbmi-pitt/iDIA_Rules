...
## simulated data public schema dates
for i in 2008-02-13 2008-02-14;
...
do
    ...
    echo INFO: Output for this run will be written to simulated-run-$1/output-$i/idia-rules-run-$i.txt;
    mkdir simulated-run-$1/output-$i;
    java -Xmx8192m -jar target/droolstest-1.0.jar $i > simulated-run-$1/output-$i/idia-rules-run-$i.txt
    sleep 1    
done
echo INFO: run completed. Assembling the resulting data and writing it to simulated-run-$1/full-simulated-run.tsv
grep -r "^DATA" simulated-run-$1/output* | sort | gcut --complement -f1 > simulated-run-$1/full-simulated-run.tsv
echo INFO: Assembling the unique resulting data and writing it to simulated-run-$1/uniq-simulated-run.tsv
grep -r "^DATA" simulated-run-$1/output* | sort | uniq | gcut --complement -f1 > simulated-run-$1/uniq-simulated-run.tsv
echo INFO: All done!
